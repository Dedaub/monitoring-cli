...
### 12.2 Independent fact-check (2026-06-02) — all claims confirmed

Cross-checked against `curvefi/curve-llamalend.js` (`aliases.ts`), `curvefi/curve-stablecoin`, Curve docs/news, and block explorers; re-confirmed on-chain:

1. **OneWayLendingFactory `0xeA6876DD…`** (Ethereum, `market_count` 48) — ✅ confirmed *verbatim* by `aliases.ts` (`OneWayFactory`); the `0x4F8846Ae…` listed in older `curve-js` is the crvUSD **Stableswap pool factory** (`pool_count` 29), not the lending factory.
2. **scrvUSD `0x0655977F…`, Yearn-V3 ERC-4626, Ethereum-only** — ✅ confirmed (`aliases.ts` `stcrvUSD`; no L2 savings address exists).
3. **9 mint markets (sfrxETH×2, wstETH, WBTC, WETH, tBTC, weETH, cbBTC, LBTC)** — ✅ confirmed: cbBTC/weETH/LBTC are the three newest additions (Curve governance + "Enhanced crvUSD Markets" announcement) atop the original six; matches the live `n_collaterals()` = 9.
4. **LlamaLend chains = ETH/ARB/OP** among the 8 (ARB `0xcaEC110C…`, OP `0x5EA8f3D6…` + V2 `0x1973ED17…`) — ✅ confirmed by `aliases.ts`; Fraxtal (`0xf3c9bdAB…`) and Sonic (`0x30d1859d…`) exist but are out of scope.
5. **Deployed `Liquidate` = 5-arg, 1-indexed** (not the master repo's 7-arg/2-indexed rewrite) — ✅ confirmed (keccak + PUSH32 bytecode scan across all 9 mint Controllers).
6. **PegKeeper v1 (4) / v2 (5) behind PegKeeperRegulator `0x36a04CAf…`** — ✅ confirmed (read live from the regulator).

**Cross-chain note:** scrvUSD's cross-chain price relies on the 2025 **Curve Block Oracle** (storage proofs) — documented in the AMM sibling's fact-check ([curve.md](curve.md) §14.1); it is not a crvUSD-specific contract. **Net corrections folded in:** the garbled weETH (ix-6) Controller/AMM addresses were re-derived live and fixed (§3.2); all other claims confirmed.
