...
## Signal identity: prefer the signature forms (readable SQL)

**Default to the signature-string forms** — the SQL then *names* what it watches (a reviewer reads
`upgradeToAndCall(address,bytes)`, not `\x4f1ef286`), and the event forms hand you **decoded, named
columns** (e.g. `implementation` from `Upgraded(address indexed implementation)`) instead of hand-decoded
topics:

1. `inputs='0x<addr>.Event(type indexed name, …)'` — pinned to one contract; adds `address=` →
   index-fast everywhere. Go-to for one known contract.
2. `inputs='Event(type indexed name, …)'` — global (topic0-wide) form; costs an extra `decode_event`
   join vs raw topic0, fine when you want the decoded columns.
3. `{{<chain>.transaction_detail("fnName(type name, …)")}}` — calls by function signature (decoded args
   by name); `{{transaction_detail('0x<addr>.fnName(types)')}}` pins one function on one contract.

**`indexed` is load-bearing** (warning below) — copy event signatures with **exact `indexed` flags** from
the protocol ref. For a form you haven't used before, confirm the expansion once with `preprocess-query`.

### Raw `topic0` — the all-forks form (always annotated)

A raw topic0 catches a protocol **and all forks** at once (one `PoolCreated` topic0 surfaced 6 V3 forks on
...
`-- fnName(types)` comment — no exceptions.** An unannotated 32-byte literal is unreviewable.
...
### No usable identity anywhere → proxy event

**Not indexed anywhere → proxy.** Scan a **co-occurring indexed event** in the same tx as the trigger. Usual proxy = the ERC-20 `Transfer` the action moves *into* the contract — index-fast on the standard `Transfer` topic0, contract pinned as recipient in `topic2`:
   ```sql
...
**`indexed` is load-bearing in the signature forms:** the macro maps params to topic vs `data` by the
`indexed` flag. Omit it on an indexed param → macro folds `topicN IS NULL` → **silent 0 rows**. Since the
signature form is the preferred default, this is THE footgun to respect: take signatures verbatim from the
protocol ref (which records the flags), and confirm with `preprocess-query` (render must show
`topicN is not null` + `"indexed": true`). If the flags are uncertain and unverifiable, fall back to the
annotated raw-`topic0` form — wrong flags lose silently, a raw topic0 can't.
