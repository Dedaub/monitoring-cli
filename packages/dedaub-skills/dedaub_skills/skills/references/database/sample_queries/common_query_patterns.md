...
| `<chain>.tx_hash(int8, int)` | `ethword` | `(block_number, tx_index)` → tx hash. **Present on all 8 chains** (verified, incl. `robinhood`) — **prefer it over a `{{outer_transaction}}` JOIN** when you only need the hash: one fewer table, and it keeps the large `outer_transaction` out of the cold working set (see `decode_primitives.md`). |
